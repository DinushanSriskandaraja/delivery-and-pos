-- ============================================================================
-- BULLETPROOF DATABASE RESET - No duplicate key errors possible
-- ============================================================================
-- This script can be run multiple times safely
-- ============================================================================

-- STEP 1: Completely disable the trigger first
...
-- STEP 4: Wait a moment (comment explains why)
-- This ensures all triggers are fully dropped before proceeding
...
-- STEP 6: Migrate ALL existing auth users (this is the key!)
DO $$
DECLARE
  auth_user RECORD;
BEGIN
  FOR auth_user IN SELECT * FROM auth.users LOOP
    ...
    VALUES (
      ...
      COALESCE(auth_user.raw_user_meta_data->>'role', 'consumer')
    )
...
-- STEP 12: Create user signup trigger (ONLY for future users)
...
-- DONE!
SELECT 'Database reset complete! All users migrated successfully.' as status;
SELECT COUNT(*) || ' users migrated' as user_count FROM users;
